  ...
  #Summarizes the information in the representation matrix in a by-node manner
{
  ...
  nodesize <- sapply(nodenums, Node_size)
  nodeoccupancy <- sapply(nodenums, Node_occupancy, comm = comm, tree = tree)
  #node_overrep_overlap <- sapply(nodenums, Node_overrep.overlap, dats = dispersion)
  #nodeoverlap <- sapply(nodenums, Node_overlap)
...
{
  desc = Descendants(node_number, tree)
